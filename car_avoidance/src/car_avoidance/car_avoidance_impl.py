...
class CarObstacleDetector(object):
    """
    Pedestrian detector, capable of checking if there is object infront of the car
    """

    def __init__(self):

    #     self.n_measurements_consider_valid = 4
        self.pub_avoidance_area = rospy.Publisher("/avoidance_area", PolygonStamped, queue_size=1, latch=True)
        self.attention_center = [5.0, 0.0, 0.0]
        self.attention_wide   = [4.0, 4.0]
        
        self.pedestrian_area = DDynamicReconfigure("avoidance_aera")
        self.pedestrian_area.add_variable('x_offset','x_offset', 
                                    self.attention_center[0], 
                                    min= self.attention_center[0] - 5.0, 
                                    max = self.attention_center[0] + 5.0)
        self.pedestrian_area.add_variable('y_offset','y_offset', 
                                    self.attention_center[1], 
                                    min= self.attention_center[1] - 5.0, 
                                    max = self.attention_center[1] + 5.0)
        self.pedestrian_area.add_variable('x_size','x_size', 
                                    self.attention_wide[0], 
                                    min= self.attention_wide[0] - 5.0, 
                                    max = self.attention_wide[0] + 5.0)
        self.pedestrian_area.add_variable('y_size','y_size', 
                                    self.attention_wide[1], 
                                    min= self.pedestrian_wide[1] - 5.0, 
